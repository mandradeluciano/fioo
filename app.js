...
function renderFeed() {
  ...
  list.querySelectorAll("[data-action-id]").forEach((el) => {
    el.addEventListener("click", () => openRecord("action", el.dataset.actionId));
  });
}
...
function renderToday() {
  ...
  let items = state.actions.filter((a) => a.status !== "concluida");
  ...
  el.querySelectorAll("[data-aid]").forEach((node) => {
    node.addEventListener("click", () => openRecord("action", node.dataset.aid));
  });
...
function currentRecord() {
  const list = modal.type === "decision" ? state.decisions : state.actions;
  return list.find((x) => x.id === modal.id) || null;
}
...
function renderModal() {
  ...
  body.innerHTML = modal.mode === "edit" ? editForm(rec)
    : (modal.type === "decision" ? decisionView(rec) : actionView(rec));
  wireModal();
}
...
function wireModal() {
  ...
  if (delBtn) delBtn.addEventListener("click", async () => {
    ...
    await deleteRecord(modal.type === "decision" ? "decisions" : "actions", modal.id);
    closeModal();
...
function ownerButtons(rec) {
  ...
  return `<button class="btn btn-ghost btn-sm" data-edit>Editar</button>
          <button class="btn btn-ghost btn-sm" data-delete>Excluir</button>`;
}

function decisionView(d) {
  const linked = state.actions.filter((a) => a.decisionId === d.id);
  const proj = projectName(d.projectId);
  return `
    ...
    <div class="modal-section" style="display:flex;gap:8px;justify-content:flex-end">${ownerButtons(d)}</div>`;
}
...
function actionView(a) {
  ...
  return `
    ...
    ${origin ? section("Nasceu da decisão", escapeHtml(origin.title)) : ""}
      ...
      <div class="capture-grid">
        <select name="type" class="type-select">
          ${Object.entries(TYPE_LABEL).map(([k, v]) =>
            `<option value="${k}" ${k === rec.type ? "selected" : ""}>${v}</option>`).join("")}
        </select>
        <select name="projectId" class="project-select">${projectOptions(rec.projectId)}</select>
      </div>
      <div class="capture-grid">
        <input name="assignee" placeholder="Responsável / com quem" value="${escapeAttr(rec.assignee)}" />
        <input name="dueDate" type="date" value="${rec.dueDate || ""}" />
      </div>
      <div class="capture-grid">
        <select name="decisionId">
          <option value="">Nasceu de qual decisão? (opcional)</option>
          ${state.decisions.map((d) =>
            `<option value="${d.id}" ${d.id === rec.decisionId ? "selected" : ""}>${escapeHtml(truncate(d.title, 60))}</option>`).join("")}
        </select>
        <select name="status">
          ${Object.entries(STATUS_LABEL).map(([k, v]) =>
            `<option value="${k}" ${k === rec.status ? "selected" : ""}>${v}</option>`).join("")}
        </select>
      </div>
      <div class="capture-footer" style="gap:8px">
        <button type="button" class="btn btn-ghost" data-cancel>Cancelar</button>
