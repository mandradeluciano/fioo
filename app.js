...
function subscribeData() {
  ...
  membershipUnsub = onSnapshot(mq, (snap) => {
    ...
    if (state.currentWs !== "all" && !wsIds.includes(state.currentWs)) state.currentWs = "all";
    ...
    renderWsSwitcher();
    render();
  }, (err) => console.error("Erro lendo memberships:", err));
}
...
async function joinByCode(code) {
  ...
  state.currentWs = inv.workspaceId;
  return inv;
}
...
function initNav() {
  ...
  $("#ws-switcher").addEventListener("change", (e) => {
    state.currentWs = e.target.value;
    render();
  });
...
function openWsModal() { $("#ws-modal").classList.remove("hidden"); renderWsModal(); }
function closeWsModal() { $("#ws-modal").classList.add("hidden"); }
